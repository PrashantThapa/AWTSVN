...
{
	public class DLLPayrollSalarySheetAdjustment
	{
		public ATTPayrollSalarySheetAdjustment ViewReport(int? OfficeCD, int? CostCenterID, int? SalYear, int? SalMonth)
		{
			...
			try
			{
				...
				ATTPayrollSalarySheetAdjustment objEmpSalary = new ATTPayrollSalarySheetAdjustment();
				...
				objEmpSalary.OriginalAmount = string.IsNullOrEmpty(drow["ORIGINAL_AMOUNT"].ToString()) ? (Int16?)null : Int16.Parse(drow["ORIGINAL_AMOUNT"].ToString());

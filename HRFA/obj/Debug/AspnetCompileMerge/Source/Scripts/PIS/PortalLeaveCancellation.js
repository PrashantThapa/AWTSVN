...
function LeaveCancellationViewModel() {
    ...
    self.EmpID = ko.observable($("#empID").text());
    ...
    self.EntryBy = ko.observable($("#userID").text());
    ...
    self.ValidateFromDate = function () {
        ...
        if (self.CancellationFromDate() == "" || self.CancellationFromDate() === undefined) {
            ...
            msg("Please fill cancel date!!!<br>", "FAILURE");
            //self.ApproveDate('');
        }
        if (self.CFromDate() == "" || self.CFromDate() === undefined) {

            //msg("कृपया विदा रद्द सूची  छान्नुहोस्! !!!<br>", "FAILURE");
            self.CFromDate("");
    ...
    self.ValidateToDate = function () {
        ...
        if (!Validate.empty(self.CancellationToDate()) && !Validate.empty(self.CToDate())) {
            ...
            console.log(self.Days());
            if (self.Days() < 0) {
                msg("Cancel end date should be small or equal!!!");
    ...
    self.ValidateDays = function () {
        ...
        if (self.CToDate() == "" || self.CToDate() === undefined) {
            //errMsg += "कृपया विदा रद्द सूची  छान्नुहोस्! !!!<br>";
        ...
        if ((self.CancellationFromDate() != "" || self.CancellationFromDate() != undefined) && (self.CancellationToDate() != "" || self.CancellationToDate() != undefined)) {
            ...
            if (Cdate < self.CancellationNoOfDays()) {
                errMsg += "Cancel total days should be smaller or equals!!!";
    ...
    self.Search = function () {
        ...
        var args = self.EmpID();
        console.log("empID", args);

        $.ajax({
            dataType: "json",
            cache: false,
            url: '../../Handlers/PIS/LeaveCancellationHandler.ashx',
            data: { 'method': 'GetPortalLeaveCancellation', 'empID': args },
            contentType: "application/json; charset=utf-8",
            success: function (result) {
    ...
    self.SaveEntity = function () {
        ...
        if (self.Validation()) {
            ...
            if (self.ValidateDays()) {
                ...
                var method = 'SavePortalLeaveCancellation';
    ...
    //    $('#modalPortalEmpSearch').on('hidden.bs.modal', function () {
    //        self.EmpID(GEmpID);
...
$(document).ready(function () {
    ...
    ko.applyBindings(new LeaveCancellationViewModel());

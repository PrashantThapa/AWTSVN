...
var OfficeEmployeeInfoViewModel = function () {
    ...
    self.ViewEmployeeReport = function () {
        $.ajax({
            dataType: "json",
    ...
    self.ViewReportDepartmentList = function () {

        $.ajax({
            dataType: "json",
            url: '/Reporting/PIS/ReportHandlers/OfficeEmployeeInfoHandler.ashx.ashx',
            data: { 'method': 'GetDepartmentList', 'officecd': self.SelectedOffice() },
            async: false,
            contentType: "application/json; charset=utf-8",
            success: function (data) {
                var mappedTask = $.map(data.ResponseData, function (item) {

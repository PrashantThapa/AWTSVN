    ...
    $.extend(calenderFunctions, {
       ...
       // alert (adYear +"--- "+ adMonth +"----"+ adDate);
            ...
            if (adDate >= 1 && adDate <= bsMonthFirstAdDate.getDate()) {
               ...
               // alert (bsMonth);
                var bsMonthDays = calenderFunctions.getBsMonthDays(bsYear, bsMonth);
                bsDate = bsMonthDays - (bsMonthFirstAdDate.getDate() - adDate) + 1; //+ 1;
            } else {
                bsDate = adDate - bsMonthFirstAdDate.getDate() + 1; //+ 1;
            }
            // alert (adDate);
            // alert( bsMonthFirstAdDate.getDate());
            return {
                bsYear: bsYear,
                bsMonth: bsMonth,
                bsDate: bsDate
...
//            formattedDate = formattedDate.replace(/%m/g, calenderFunctions.getNepaliNumber(bsMonth));
			...
			formattedDate = formattedDate.replace(/%d/g, calenderFunctions.pad(bsDate,2));
            formattedDate = formattedDate.replace(/%y/g, bsYear);
            formattedDate = formattedDate.replace(/%m/g, calenderFunctions.pad(bsMonth+1,2));
        ...
        var datePickerPlugin = {
            ...
            addEventHandler: function ($element, $nepaliDatePicker) {
                $element.focus(function () {
                  ...
                  //  var inputFieldPosition = $(".bod-picker").offset();
                    var inputFieldPosition = $(this).offset();
                    //alert(inputFieldPosition.top);
                    $nepaliDatePicker.css({
                        "top": inputFieldPosition.top + 25,
                ...
                // added to hide datepicker when click out side of the text box
				$( document ).on( 'click', function ( e ) {
                  // 
					var container = $(".nepali-date-picker");
                      var container1 = $("bod-picker");
                    
                     if (e.target.className.indexOf('bod-picker') >0)
                      {
                      return; 
                      }
                      //
...
//						            datePickerPlugin.eventFire($element, $nepaliDatePicker, "hide");
//					            }	
                            //}

                            if ($(e.target).closest(".calender-wrapper").length === 0) {
                                    $nepaliDatePicker.hide();
                ...
                // added to hide datepicker when esc key is pressed
            ...
            renderCurrentMonthCalender: function ($nepaliDatePicker) {
                ...
                //alert ("curr"+ "--" + currentDate.getFullYear()+ "--" +currentDate.getMonth()+ "--" +currentDate.getDate());
